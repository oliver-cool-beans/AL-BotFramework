...
const targets = ["scorpion"];

async function scorpion(bot, party, merchant, args) {
    ...
    const rallyPosition = "scorpion";

    if(!bot.runningScriptName == "scorpion") {
        bot.runningScriptName = "scorpion"
    ...
    if(!bot.target || !checkTarget(bot?.target, bot.character.entities)) {
        ...
        if(!bot.target) await bot.character.smartMove("scorpion").catch(() => {});
    }
