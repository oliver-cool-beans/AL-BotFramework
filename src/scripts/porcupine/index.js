...
const targets = ["porcupine"];

async function porcupine(bot, party, merchant, args) {
    ...
    const rallyPosition = "porcupine";

    if(!bot.runningScriptName == "porcupine") {
        bot.runningScriptName = "porcupine"
...
;
    ...
    if(!bot.character.target) {
        await bot.character.smartMove("porcupine").catch(() => {});
    }
