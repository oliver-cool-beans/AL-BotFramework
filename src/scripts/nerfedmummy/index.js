...
const targets = ["nerfedmummy"];

async function nerfedmummy(bot, party, merchant, args) {
    ...
    const rallyPosition = "nerfedmummy";

    if(!bot.runningScriptName == "nerfedmummy") {
        bot.runningScriptName = "nerfedmummy"
...
;
    ...
    if(!bot.character.target) {
        await bot.character.smartMove("nerfedmummy").catch(() => {});
    }
