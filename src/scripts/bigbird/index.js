...
async function bigbird(bot, party, merchant, args) {
    ...
    if(bot.characterClass == "merchant") return Promise.resolve("Not a combigbird class");
    ...
    if(!bot.target || !checkTarget(bot?.target, bot.character.entities)) {
        bot.target = utils.findClosestTarget(bot.AL, bot.character, party, targets, true, true, true);
        if(!bot.target) await bot.character.smartMove("bigbird").catch(() => {});
    }
