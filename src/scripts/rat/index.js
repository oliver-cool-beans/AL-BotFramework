...
async function rat(bot, party, merchant, args) {
    ...
    const rallyPosition = "rat";

    if(!bot.runningScriptName == "rat") {
        bot.runningScriptName = "rat"
    ...
    if(!bot.target || !checkTarget(bot?.target, bot.character.entities)) {
        ...
        if(!bot.target) await bot.character.smartMove("rat").catch(() => {});
    }
